...
int main() {
	...
	while (1) {
		...
		buf[ret - 1] = 0;
		if (!isLoggedIn) {
			FILE* f = fopen("D:\\Downloads\\20212\\Network-Programming\\Account.txt", "r");
			char account[100];
			while (!feof(f)) {
			...
			strcat(buf, " > D:\\Downloads\\20212\\Network-Programming\\Result.txt");
			system(buf);
			send(client, "Success! See response at directory D:\\Downloads\\20212\\Network-Programming\\Result.txt\n", 86, 0);
		}
	}
}